                ...
                <div class="card border border-light">
                    <div class="card-body">
                        ...
                        <div class="row d-flex justify-content-between align-items-center me-1">
                                    ...
                                    <div class="modal-dialog center">
                                        <div class="modal-content">
                                            ...
                                            <div class="modal-body">
                                                <form id="recensementForm" action="{{ route('tpu.recense') }}" method="POST">
                                                    @csrf
                            ...
                            <div class="col-md-2">
                                <a href="{{ route('tpu.imprimer') }}" target="_blank" class="btn btn-outline-success btn-sm-lg d-flex align-items-center justify-content-center gap-1">Imprimer <i class="bx bx-printer"></i></a>
                            </div>
                            <div class="col-md-4 ms-auto">
                                <form method="GET" action="{{ route('tpu.recherche') }}">
                        ...
                        <div class="table-responsive">
                            <table class="table table-bordered table-hover table-striped " id="recensementsTable">
                                ...
                                <tbody>
                                    @foreach ($recencement_tpu as $key=> $recencement_tpus )
                                    <tr>
                                        <td>{{ $key+1 }}</td>
                                        <td>{{ $recencement_tpus->bien->contribuable->prenom.' '.$recencement_tpus->bien->contribuable->nom }}</td>
                                        <td>{{ $recencement_tpus->bien->libelle }}</td>
                                        <td>{{$recencement_tpus->bien->numero_bien  }}</td>
                                        <td>{{ $recencement_tpus->Date_recensement }}</td>
                                        <td>{{ $recencement_tpus->categorie }}</td>
                                        <td class="d-flex justify-content-center gap-2">
                                            <a href="{{route('tpu.voir',$recencement_tpus->uuid)}}" class="btn btn-outline-success btn-sm d-flex align-items-center gap-1">Voir<i class="bx bx-show"></i></a>
                                            <a href="{{route('impot.imposition',['type' => 'tpu', 'uuid' => $recencement_tpus->uuid])}}" class="btn btn-outline-success btn-sm d-flex align-items-center gap-1">Imposer<i class="bx bx-money"></i></a>
                                            <a href="{{route('tpu.modif',$recencement_tpus->uuid)}}" class="btn btn-outline-success btn-sm d-flex align-items-center gap-1">Modifier<i class="bx bx-edit"></i></a>
                                        </td>
                                    </tr>
                                    @endforeach
                                    @if(count($recencement_tpu) == 0)
                                    <tr>
                            ...
                            <div class="d-flex justify-content-center mt-3">
                                {{ $recencement_tpu->links('pagination::bootstrap-4') }}
                            </div>
                        </div>
                    </div>
                </div>
